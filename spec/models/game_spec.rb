RSpec.describe Game, type: :model do
    ...
    context '.build' do
        ...
        it 'should creates new Game' do
            expect { Game.build(user_1.id, user_2.id, true) }.to change(Game, :count).by(1)
        end

        it 'user_1 is owner, user_2 is opponent and game is opened' do
            game = Game.build(user_1.id, user_2.id, true)
        ...
        it 'and creates new Board' do
            expect { Game.build(user_1.id, user_2.id, true) }.to change(Board, :count).by(1)
        end

        it 'and creates new 32 Figures' do
            expect { Game.build(user_1.id, user_2.id, true) }.to change(Figure, :count).by(32)
        end
    end
end
